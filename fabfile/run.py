...
def pgbench_tests(config_file='pgbench_default.ini', connectionURI=''):
    ...
    results_file = open(os.path.join(config.HOME_DIRECTORY, 'pgbench_results_{}.csv'.format(current_time_mark)), 'w')
...
def tpch_queries(query_info, connectionURI, pg_version, citus_version):
    results_file = open(
        os.path.join(config.HOME_DIRECTORY,'tpch_benchmark_results_PG-{}_Citus-{}.txt'.format(pg_version, citus_version)),
        'a')
